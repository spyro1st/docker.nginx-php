...
RUN echo "deb http://dotdeb.netmirror.org/ wheezy-php55 all" >> /etc/apt/sources.list
RUN echo "deb-src http://dotdeb.netmirror.org/ wheezy-php55 all" >> /etc/apt/sources.list

RUN apt-get update -qq && \
    apt-get install -qqy supervisor openssh-server git curl procps cron unzip nginx-extras mysql-client vim-tiny php5 php5-cli php5-common php5-intl php5-curl php5-fpm php5-gd php5-imagick php5-mcrypt php5-memcache php5-mysqlnd php-pear php5-xsl php5-xdebug graphicsmagick ssl-cert ssmtp && \
